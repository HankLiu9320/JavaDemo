...
public class CommandHelloWorld2 extends HystrixCommand<String> {
    ...
    public static void main(String[] args) throws Exception {
        ...
        /**
         * 注意： 1.除了HystrixBadRequestException异常之外，所有从run()方法抛出的异常都算作失败，
         ...
         * 可以使用HystrixThreadPoolKey做隔离区分. 最然在业务上都是相同的组，但是需要在资源上做隔离时，
         * 可以使用HystrixThreadPoolKey区分.
         * 
         * 6.以下四种情况将触发getFallback调用： 1.)run()方法抛出非HystrixBadRequestException异常。
         * 2.)run()方法调用超时 3.)熔断器开启拦截调用 4.)线程池/队列/信号量是是否跑满
         * 实现getFallback()后,执行命令时遇到以上4中情况将被fallback接管, 不会抛出异常或其他。
         */
    }
}